export default {
	...
	methods: {
		async checkNetwork() {
			const device = await this.$systeminformation.networkInterfaces(
				'default'
			);
			...
			if (device.type === 'wired') {
				...
				if (device.operstate === 'down') {
					...
					const wifi =
						await this.$systeminformation.wifiConnections();
					const myWifi = wifi.find(
						(conn) => conn.iface === device.iface
					);
